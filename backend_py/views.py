...
async def user_deposit_handler(request: Request, item: RequestDepositItem, auth: Annotated[dict, Depends(authentication)]):
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            a = cur.rowcount
            await cur.execute("INSERT INTO deposits (user_id, balance_add, pporder_id) VALUES (%s, %s, %s);", (user_id, item.balance_add, item.pporder_id))
            b = cur.rowcount
            if (a+b) > 0:
                await conn.commit()
                return res
...
async def get_deposits_handler(request: Request, auth: Annotated[dict, Depends(authentication)]):
    admin_id_real = auth["rid"]
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute("SELECT user_id, balance_add, pporder_id FROM deposits ORDER BY deposit_id DESC;")
            deposits = await cur.fetchall()
            res["data"]["deposits"] = list([dict(zip(["user_id", "balance_add", "pporder_id"], deposit)) for deposit in deposits])
            return res