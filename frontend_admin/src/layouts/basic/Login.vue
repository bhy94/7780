<template>
  <div class="tw-w-screen tw-h-screen tw-flex-row tw-bg-gray-100 tw-flex tw-justify-center tw-items-center">
		<div class="tw-bg-gray-50 tw-rounded-2xl tw-flex tw-flex-col tw-items-center tw-justify-center" style="width:300px; height:400px">
			<p class="tw-text-xl"> Vendor Login </p>
...
const login = () => {
	...
	axios.post('/api/login', {
		...
		destination: "back_end"
	})
	.then((resp) => {
		console.log(resp)
		if (resp.data.code == 200 && resp.data.data.success == 1) {
			Cookies.set("token", resp.data.data.token, {expires: 30});
			Cookies.set("vendor_username", inputUsername.value, {expires: 30});
			Cookies.set("vendor_id", resp.data.data.vendor_id, {expires: 30});
